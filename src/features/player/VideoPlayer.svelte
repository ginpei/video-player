<script lang="ts">
  ...
  function calculateSwipeOffset(distance: number): number {
    ...
    const acceleratedOffset =
      PLAYBACK_CONFIG.SWIPE_ACCELERATION * absDistance * absDistance;
  ...
  function handleVideoPointerMove(event: PointerEvent) {
    ...
    if (
      !isSwiping &&
      absHorizontal > PLAYBACK_CONFIG.SWIPE_DETECTION_THRESHOLD
    ) {
  ...
  function handleVideoPointerUp() {
    ...
    if (clickTimer) {
      ...
      const isRightSide =
        offsetX > rect.width * PLAYBACK_CONFIG.RIGHT_ZONE_RATIO;
      if (isLeftSide || isRightSide) {
        seekBy(
          isLeftSide
            ? -PLAYBACK_CONFIG.SEEK_AMOUNT
            : PLAYBACK_CONFIG.SEEK_AMOUNT,
        );
...
<div class="VideoPlayer w-full mx-auto mb-32">
  <div
    class={`relative overflow-hidden rounded border border-slate-800 bg-slate-950/80 shadow-[0_24px_80px_-40px_rgba(15,23,42,0.9)] ${
  ...
  >
    ...
    {#if overlay.show}
      ...
      >
        <div
          class={`absolute inset-y-0 flex items-center justify-center bg-slate-950/40 text-4xl font-semibold text-slate-100 tabular-nums ${
    ...
    {#if !videoUrl}
      ...
      >
        <div class="text-center px-6">
          ...
          <p class="mt-2 text-sm text-slate-300">Or click here to choose one</p>
        </div>
      </div>
    {/if}
    ...
    {#if isDragging}
      <div
        class="absolute inset-0 flex items-center justify-center bg-amber-400/10"
      >
        <p
          class="text-sm font-semibold uppercase tracking-[0.3em] text-amber-200"
        >
          Release to load
        </p>
      </div>
  ...
  <div class="mt-4 flex flex-col gap-3">
    <div class="card flex flex-wrap items-center gap-3 px-4 py-3">
      ...
      <div
        class="order-1 sm:order-2 flex w-full sm:min-w-45 sm:flex-1 items-center gap-3 text-sm text-slate-200"
      >
      ...
      <!-- Time displays -->
      <span class="order-3 tabular-nums text-sm text-slate-200"
